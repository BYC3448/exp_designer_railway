...
**그래프 영역 작성 지침:**
실험의 특성에 따라 x축과 y축이 무엇인지 추출하여 다음과 같은 그래프 형식을 사용하세요:
"## 데이터 그래프" 섹션 바로 아래에, 다음에 오는 Base64로 인코딩된 SVG 이미지 HTML 태그를 **절대로 변경하거나 생략하지 말고, 코드 그대로 복사하여 응답에 포함해야 한다.** 이는 학생이 데이터를 그릴 빈 좌표 평면을 제공하기 위한 필수 요소이다.
